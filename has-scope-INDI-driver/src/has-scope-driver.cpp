...
void HASScope::ISGetProperties(const char *dev)
{
...
bool HASScope::updateProperties()
{
...
bool HASScope::Connect()
{
    LOG_INFO("HAS Scope is online.");
    SetTimer(POLLMS);
    return true;
}

bool HASScope::Disconnect()
{
    LOG_INFO("HAS Scope is offline.");
    return true;
}
...
bool HASScope::Goto(double r, double d)
{
    StartSlew(r, d, SCOPE_SLEWING);
    return true;
}

bool HASScope::Sync(double ra, double dec)
{
    currentRA  = ra;
    currentDEC = dec;
...
void HASScope::StartSlew(double ra, double dec, TelescopeStatus status)
{
    targetRA  = ra;
    targetDEC = dec;
    char RAStr[64], DecStr[64];
...
bool HASScope::UnPark()
{
    SetParked(false);
    return true;
}

bool HASScope::ISNewNumber(const char *dev, const char *name, double values[], char *names[], int n)
{
...
bool HASScope::ISNewSwitch(const char *dev, const char *name, ISState *states, char *names[], int n)
{
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        // Slew mode
...
bool HASScope::MoveNS(INDI_DIR_NS dir, TelescopeMotionCommand command)
{
    INDI_UNUSED(dir);
    INDI_UNUSED(command);
...
bool HASScope::MoveWE(INDI_DIR_WE dir, TelescopeMotionCommand command)
{
    INDI_UNUSED(dir);
    INDI_UNUSED(command);
    if (TrackState == SCOPE_PARKED)
...
IPState HASScope::GuideNorth(uint32_t ms)
{
...
IPState HASScope::GuideSouth(uint32_t ms)
{
...
IPState HASScope::GuideEast(uint32_t ms)
{
...
IPState HASScope::GuideWest(uint32_t ms)
{
...
bool HASScope::SetTrackMode(uint8_t mode)
{
    INDI_UNUSED(mode);
    return true;
}

bool HASScope::SetTrackEnabled(bool enabled)
{
    INDI_UNUSED(enabled);
    return true;
}

bool HASScope::SetTrackRate(double raRate, double deRate)
{
    INDI_UNUSED(raRate);
    INDI_UNUSED(deRate);
    return true;
